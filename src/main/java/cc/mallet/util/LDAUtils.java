...
public class LDAUtils {
	...
	public static String[][] getTopDistinctiveWords(int noWords, int numTypes, int numTopics, 
		...
		double [] p_t = calcTopicProb(typeTopicCounts);

		double [][] distinctiveness = calcWordDistinctiveness(p_w_k, p_t);
	...
	public static String[][] getTopSalientWords(int noWords, int numTypes, int numTopics, 
		...
		double [][] saliency = calcWordSaliency(p_w_k,p_w, p_t);
	 ...
	 */
	public static double[][] calcWordDistinctiveness(double [][] p_w_k, double [] p_t) {
		...
		for (int w = 0; w < nrWords; w++) {
			for (int k = 0; k < nrTopics; k++) {
				wordDistinctiveness[w][k] += p_w_k[w][k] * log(p_w_k[w][k] / p_t[k]);
			}
		}
	 ...
	 */
	public static double[][] calcWordSaliency(double [][] p_w_k, double [] p_w, double [] p_t) {
		int nrTopics = p_w_k[0].length;
		int nrWords = p_w_k.length;
		double [][] wordDistinctiveness = calcWordDistinctiveness(p_w_k, p_t);
