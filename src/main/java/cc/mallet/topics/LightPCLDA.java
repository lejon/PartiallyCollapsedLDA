...
public class LightPCLDA extends SpaliasUncollapsedParallelLDA {
	...
	protected void sampleTopicAssignmentsParallel(LDADocSamplingContext ctx) {
		...
		for (int i = 0; i < localTopicCounts.length; i++) {
			if(localTopicCounts[i]!=0) nonZeroTopicCnt++;
		}
		...
		for (int position = 0; position < docLength; position++) {
			...
			//System.out.println("Word Acceptance rate : " + pi_w);
			...
			//System.out.println("Doc Acceptance rate : " + pi_d);

			double u_pi_d = ThreadLocalRandom.current().nextDouble();
			boolean accept_pi_d = u_pi_d < pi_d;
			
			if(ThreadLocalRandom.current().nextDouble() > 0.5 && accept_pi_w) {				
