...
/**
 * This version of TopicModelDiagnostics takes a dense (while the original takes a sparse) type-topic matrix
 * It also uses the LDAGibbsSampler interface
 * 
 * @author Leif Jonsson
 *
 */
public class TopicModelDiagnosticsPlain {
	...
	public TopicModelDiagnosticsPlain (LDAGibbsSampler model, int numTopWords) {
		...
		topicSortedWords = LDAUtils.getSortedWordsDense(model.getNoTopics(), model.getNoTypes(), model.getTypeTopicMatrix());
		...
		diagnostics.add(getTokensPerTopic(model.getTopicTotals()));
		diagnostics.add(getDocumentEntropy(model.getTopicTotals()));
			...
			if (docLength > 0) {
				...
				for (int topic = 0; topic < numTopics; topic++) {
					...
					if (topicCounts[topic] > 0) {
						...
						double proportion = (alpha[topic] + topicCounts[topic]) / (alphaSum + docLength);
	...
	public TopicScores getDistanceFromUniform() {
		int[] tokensPerTopic = model.getTopicTotals();
		...
		for (int topic = 0; topic < numTopics; topic++) {
			...
			for (int row = 0; row < numTopWords; row++) {
				...
				for (int col = 0; col < row; col++) {
					double score = Math.log( (matrix[row][col] + model.getBeta()) / (matrix[col][col] + model.getBeta()) );
					rowScore += score;
	...
	public String topicsToCsv() {
		StringBuilder out = new StringBuilder();
		Formatter formatter = new Formatter(out, Locale.US);
		formatter.format("Topic,");
		int cnt = diagnostics.size();
		for (TopicScores scores: diagnostics) {
			formatter.format("%s", scores.name);
			if(cnt>1) {
				formatter.format(",");
				cnt--;
			}
		...
		formatter.format("\n");
		for (int topic = 0; topic < numTopics; topic++) {
			cnt = diagnostics.size();			
			formatter.format("%d,", topic);

			for (TopicScores scores: diagnostics) {
				formatter.format("%.4f", scores.scores[topic]);
				if(cnt>1) {
					formatter.format(",");
					cnt--;
				}
