 ...
 * 
 * Same as the original in MALLET but without the sparse encoding of the type-topic matrix
 * 
 * @author David Mimno
 * @author Leif Jonsson
 */

public class MarginalProbEstimatorPlain implements Serializable {
	...
	public MarginalProbEstimatorPlain (int numTopics,
	...
	protected double[] leftToRight (FeatureSequence tokenSequence) {
		// The resampling implementation is broken (ALSO THE ORIGINAL MALLET VERSION)
		...
		for (int limit = 0; limit < docLength; limit++) {
			...
			// WITH RESAMPLING IS BROKEN (FROM MALLET)
			if (usingResampling) {
    ...
    public static MarginalProbEstimatorPlain read (File f) throws Exception {

        MarginalProbEstimatorPlain estimator = null;

        ObjectInputStream ois = new ObjectInputStream (new FileInputStream(f));
        estimator = (MarginalProbEstimatorPlain) ois.readObject();
        ois.close();
