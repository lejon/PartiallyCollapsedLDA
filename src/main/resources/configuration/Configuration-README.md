...
Decides which words to sample in Phi
...
# Save Phi mean:
Must be set for output to be created
```
save_phi_mean = true
...
## Percentage Split Size_doc
```
percentage_split_size_doc
```

## Percentage Split Size_topic
```
percentage_split_size_topic
```
...
## Print Ndocs Interval
```
print_ndocs_interval
```
...
## Print Ntopwords Interval
```
print_ntopwords_interval
```
...
## Term Frequencies Filename
```
term_frequencies_filename
```
...
## Doc Lengths Filename
```
doc_lengths_filename
```

## Sparse Dirichlet Sampler Builder Name
```
sparse_dirichlet_sampler_builder_name
```
...
## Hyperparam Optim Interval
```
hyperparam_optim_interval
```
